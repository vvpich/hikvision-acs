...
- Фото приходит только если терминал прикладывает снимок к событию. На
  DS-K1T342MFWX авторизация картой приходит **без** JPEG — камера в этом
  случае пуста, и это поведение устройства, а не интеграции. Проверить
  можно по debug-логу: строки `JPEG из потока, N байт`.
