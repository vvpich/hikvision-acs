...
Из официального Hikvision ISAPI Developer Guide for Face Recognition
Terminals — на DS-K1T342MFWX не наблюдались, но оставлены для других
прошивок:
...
Коды заметно расходятся между прошивками: та же карта у одного терминала
приходит как `5/1`, у другого как `5/38`. Свои смотри в debug-логе.

`door_opened` и `door_closed` прилетают через доли секунды после прохода.
Они публикуются как события, но фото и сенсоры «последний
пользователь/метод» не трогают — те обновляются только авторизациями
(`AUTH_EVENT_TYPES` в `const.py`).
...
Чтобы снимок дошёл до Home Assistant, на терминале должны быть выполнены
**два независимых условия**. Пока не выполнены оба, `camera.*` остаётся
пустой, и по логам это выглядит одинаково — JPEG в поток просто не
приходит.

### 1. Правило привязки, которое делает снимок

Снимок делает не событие само по себе, а *привязка* (event card linkage).
В веб-интерфейсе: **Система и обслуживание → Событие → Настройка связи**,
кнопка **+**, тип связи **Привязка события**, нужный тип события, и
включить **Связанный снимок**. Через ISAPI это
`/ISAPI/AccessControl/eventCardLinkageCfg/<id>` (id от 1 до 50) с
`capturePic: true`.

Коды в привязках живут в **своём** пространстве и не совпадают с
`majorEventType/subEventType` из потока. Снято с DS-K1T342MFWX V4.38.0:

| Привязка | Событие в потоке |
|---|---|
| `mainEventType: 3, subEventType: 26` | `5/76` — распознавание лица не удалось |
| `mainEventType: 3, subEventType: 25` | `5/75` — распознавание лица (предположительно) |
| `mainEventType: 2, subEventType: 6` | событие двери |

Полного сопоставления названий и кодов нет ни в ISAPI, ни в руководстве
пользователя (там только «select event types according to your actual
needs»). Надёжный способ узнать код — создать правило в веб-интерфейсе,
выбрав событие по названию, и прочитать записанный `subEventType` через
ISAPI.

### 2. Флаги AcsCfg

Управляется через `/ISAPI/AccessControl/AcsCfg`:

| Флаг | Что делает |
|---|---|
| `uploadCapPic` | загружать снимок связанного захвата вместе с событием |
| `uploadVerificationPic` | загружать снимок верификации (лицо/карта) |
| `saveCapPic` | хранить снимки захвата на устройстве |
| `saveVerificationPic` | хранить снимки верификации на устройстве |

На заводских настройках DS-K1T342MFWX все они `false`, поэтому события
приходят без фото и `camera.*` остаётся пустой. Посмотреть текущее
состояние и поддерживаемые значения:

```bash
curl -sk --digest -u ЛОГИН:ПАРОЛЬ \
  "https://IP:443/ISAPI/AccessControl/AcsCfg?format=json"
curl -sk --digest -u ЛОГИН:ПАРОЛЬ \
  "https://IP:443/ISAPI/AccessControl/AcsCfg/capabilities?format=json"
```

Включить все четыре (`save*` обязательны: без них терминал снимок не
сохраняет, и отправлять оказывается нечего — в журнале событий тогда нет
даже `pictureURL`):

```bash
curl -sk --digest -u ЛОГИН:ПАРОЛЬ -X PUT \
  -H "Content-Type: application/json" \
  -d '{"AcsCfg":{"uploadCapPic":true,"uploadVerificationPic":true,
                 "saveCapPic":true,"saveVerificationPic":true}}' \
  "https://IP:443/ISAPI/AccessControl/AcsCfg?format=json"
```

Интеграция проверяет эти флаги при запуске и пишет в лог предупреждение,
если загрузка или сохранение снимков выключены.

### Как убедиться, что работает

Включи debug-логи интеграции и сделай проход. В журнале должно быть
событие и сразу за ним JPEG:

```
Событие face_authenticated (major=5 minor=75)
JPEG из потока, 28266 байт
```

Проверено на DS-K1T342MFWX V4.38.0: при выполнении обоих условий снимок
приходит вложением в поток и попадает в `camera.*`.

Часть прошивок умеет вместо вложения отправлять снимки на внешний сервер
(`/ISAPI/System/PictureServer`) — тогда в событии будет ссылка, а не JPEG,
и этот режим интеграция не поддерживает.
...
- Сенсор `last_user` показывает имя последней **успешной** авторизации и
  держит его до следующего прохода; отказы и события двери его не меняют.
- Фото приходит только если на терминале включена загрузка снимков — см.
  раздел «Фото в событиях». Проверить можно по debug-логу: строки
  `JPEG из потока, N байт`.
